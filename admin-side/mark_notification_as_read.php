...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['item_id'], $_POST['notif_type'], $_POST['reservation_type'])) {
        ...
        if ($notifType === 'appointments') {
            ...
            switch ($_POST['reservation_type']) {
                case 'reception':
                    ...
                    break;
                case 'recollection':
                    $updateQuery = "UPDATE recollection_reservation_record SET is_read = 1 WHERE recollection_id = :itemId";
                    break;
                case 'retreat':
                    $updateQuery = "UPDATE retreat_reservation_record SET is_read = 1 WHERE retreat_id = :itemId";
                    break;
                case 'seminar':
                    $updateQuery = "UPDATE seminar_reservation_record SET is_read = 1 WHERE seminar_id = :itemId";
                    break;
                case 'training':
                    $updateQuery = "UPDATE training_reservation_record SET is_read = 1 WHERE training_id = :itemId";
                    break;
                default:
