...
$sqlCount = "SELECT COUNT(*) AS total_reservations FROM (
   ...
   $row = $result->fetch_object();
    ...
    $sqlCount2 = "SELECT COUNT(*) AS paid FROM (
                ...
                UNION ALL
                SELECT * FROM training_reservation_record
            ) AS all_reservations WHERE status = 'confirmed'";
    $result2 = $conn->query($sqlCount2);
   $row = $result2->fetch_object();
    $paid = $row->paid;
     $sqlCount3 = "SELECT COUNT(*) AS pending FROM (
                SELECT * FROM reception_reservation_record
                UNION ALL
                SELECT * FROM recollection_reservation_record
                UNION ALL
                ...
                UNION ALL
                SELECT * FROM training_reservation_record
            ) AS all_reservations WHERE status = 'pending'";
...
$pdf->Cell(1 ,5,'',0,1);
$pdf->Cell(20 ,5,'',0,0);
$pdf->Cell(100 ,5,'Pending Payment: '.$pending.'',0,1);//end of line
$pdf->Cell(1 ,5,'',0,1);
$pdf->Cell(20 ,5,'',0,0);
$pdf->Cell(100 ,5,'Total Sales of Paid Payment: '.$sumpaid.'',0,1);//end of line
$pdf->Cell(1 ,5,'',0,1);
$pdf->Cell(20 ,5,'',0,0);
$pdf->Cell(100 ,5,'Total Sales of Pending Payment: '.$sumpend.'',0,1);//end of line
$pdf->Cell(1 ,5,'',0,1);
$pdf->Cell(20 ,5,'',0,0);
$pdf->Cell(100 ,5,'TOTAL SALES: '.$total.'',0,1);//end of line
$pdf->Output();
